...
// показывалось 10 556 690 - разница 40 000. REFERENCE ниже - точная копия списка (номер
// заказа, Сумма) из выгрузки 1С. Функция читает архив Заказы_2026-07, строит свой список по
// Савиток и построчно сверяет: каких заказов номеров у нас нет, где разошлась сумма, и
// какие заказы есть у нас, но нет в выгрузке 1С. Удалить после разбора причины.
var SAVITOK_JULY_REFERENCE = [
  ["468973",40000],
  ["469109",115000],
  ["469112",50000],
  ["469113",33000],
...
function diagnoseSavitokJuly() {
  ...
  const rows = parseOrdersRawRows(archive.getDataRange().getValues()).rows;
  Logger.log('Всего строк в архиве Заказы_2026-07: ' + rows.length);

  const refMap = {};
  let refTotal = 0;
  SAVITOK_JULY_REFERENCE.forEach(function(r) { refMap[r[0]] = r[1]; refTotal += r[1]; });

  const ourMap = {};
  let ourTotal = 0, ourRows = 0;
  rows.forEach(function(row) {
    const mgr = String(row[15] || '').trim();
    if (mgr.indexOf('Савиток') < 0) return;
    ourRows++;
    const id = String(row[0] || '').trim();
    const amount = parseFloat(row[30]) || 0;
    ourMap[id] = (ourMap[id] || 0) + amount;
    ourTotal += amount;
  });

  Logger.log('Строк у нас (Заказы_2026-07, Менеджер продаж содержит "Савиток"): ' + ourRows);
  Logger.log('Сумма по выгрузке 1С (эталон): ' + refTotal);
  Logger.log('Сумма у нас: ' + ourTotal);
  Logger.log('Разница (эталон минус у нас): ' + (refTotal - ourTotal));

  const missing = [], mismatched = [], extra = [];
  Object.keys(refMap).forEach(function(id) {
    if (!(id in ourMap)) missing.push(id + ' (' + refMap[id] + '₽)');
    else if (Math.abs(ourMap[id] - refMap[id]) > 1) mismatched.push(id + ': 1С=' + refMap[id] + ' у нас=' + ourMap[id]);
  });
  Object.keys(ourMap).forEach(function(id) {
    if (!(id in refMap)) extra.push(id + ' (' + ourMap[id] + '₽)');
  });

  Logger.log('ОТСУТСТВУЮТ у нас, есть в 1С (' + missing.length + '): ' + (missing.join(', ') || '-'));
  Logger.log('НЕСОВПАДЕНИЯ по сумме (' + mismatched.length + '): ' + (mismatched.join('; ') || '-'));
  Logger.log('ЕСТЬ У НАС, НЕТ в выгрузке 1С (' + extra.length + '): ' + (extra.join(', ') || '-'));
}
