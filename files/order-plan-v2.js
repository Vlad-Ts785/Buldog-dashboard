...
/* Особые условия 1-13 - фиксированный юридический текст, в основе - образец Влада
   (dogovor-zayavka-template-2026-08-26.docx), с тремя добавлениями 21.09 (п.2 - акцепт,
   п.10 - способы оплаты/простой при неоплате, п.11 - право приостановки/удержания груза
   при неоплате на погрузке/выгрузке). Меняется только по явной просьбе Влада, не по данным
   заявки. Порядок продуман как единая логика: 1-2 - как документ приобретает силу; 3-4 -
   базовые обязанности сторон; 5-9 - риски/штрафы по ходу перевозки; 9-11 - расчёты и что
   делать при неоплате; 12 - доп.расходы; 13 - споры. Час на оплату (п.10, в/г) считается
   ОТ ПРИБЫТИЯ на адрес погрузки/выгрузки (Влад, 21.09), не от завершения процедуры - не
   путать со свободными часами простоя в п.5 (те про саму погрузку/разгрузку, это про
   ожидание оплаты после прибытия, разные счётчики времени). П.6 (непредоставление груза,
   Влад 21.09, доработано 22.09) - СВОЯ бесплатная пауза 2 часа (короче общих 8 часов п.5 -
   это сценарий "груза вообще нет", не "грузят медленно"). ПОСЛЕ 2 часов - выбор за
   ИСПОЛНИТЕЛЕМ, не автоматика на 24 часа: можно продолжать ждать (копится почасовой
   простой по ставке п.5), а можно в ЛЮБОЙ момент после 2 часов уехать и зафиксировать
   срыв - тогда к уже накопленному простою добавляются 20% (Влад, 22.09: "если в два часа
   нас не грузят, мы можем спокойно уехать и выставить им срыв" - раньше пункт ОБЯЗЫВАЛ
   ждать 24 часа, прежде чем это признавалось срывом, что не давало гибко решить, стоит ли
   держать машину на этом объекте или отправить на другой рейс). 24 часа остались только
   backstop'ом - если ИСПОЛНИТЕЛЬ почему-то продолжает ждать так долго, срыв фиксируется
   автоматически, без права тянуть дальше. Простой + 20% по-прежнему СУММИРУЮТСЯ (не
   выбор одного из двух) - это разные виды потерь (простой - цена стоящей машины, 20% -
   цена сорванного рейса), Влад подтвердил именно это распределение. Правовая база
   п.11 проверена
   21.09 (не выдумана): право приостановить своё исполнение при неполучении оплаты за
   погрузку - п.2 ст.328 ГК РФ; право удержания груза перевозчиком в обеспечение провозной
   платы - п.4 ст.790 ГК РФ (прямая цитата: "Перевозчик имеет право удерживать переданные
   ему для перевозки грузы... в обеспечение причитающихся ему провозной платы и других
   платежей по перевозке") + ст.359-360 ГК РФ - это право у перевозчика есть ПО ЗАКОНУ
   независимо от договора, п.11 его не создаёт, а явно фиксирует и привязывает к
   конкретному триггеру (неоплата за час). ВАЖНО: индекс идущего под номером динамического
   пункта о простое (п.5, сейчас index 4) захардкожен в drawContractPdf_
   (`specialTerms[4] = idleTermText_(...)`) - при добавлении/удалении пунктов ВЫШЕ него
   в этом массиве обязательно поправить и тот индекс тоже, иначе подмена попадёт не в тот
   пункт (текущая правка добавляет пункт НИЖЕ index 4 - индекс 4 не трогать). Внутренние
   ссылки на номера пунктов (п.5 в №6, п.5 в №10, подпункты (в)/(г) п.10 в №11) сверены под
   этот порядок - при любой перестановке массива сверить их заново, не полагаться на
   память.

   22.09, юридический разбор всех 13 пунктов по просьбе Влада ("проверь как выпускник
   МГУ юрфака") - что исправлено и что сознательно оставлено как риск. ВНИМАНИЕ: номера
   пунктов ниже - по РЕДАКЦИИ 1 (13 пунктов); с 21.09 (вечер) действует редакция 2 из 16
   пунктов с другой нумерацией (см. комментарий прямо над массивом), история оставлена как
   объяснение, ПОЧЕМУ те решения приняты - они перенесены в новый текст:
   - п.13 (споры) - ГЛАВНАЯ находка: было "5 календарных дней" на ответ по претензии -
     это МЕНЬШЕ обязательного срока по ст.40 Устава автомобильного транспорта (259-ФЗ от
     08.11.2007) - 30 дней на рассмотрение претензии перевозчиком, проверено по первоисточнику
     (не по памяти). Если бы дошло до суда, реальный срок всё равно определял бы закон, а
     не договор - поправлено на 30 дней, добавлено явное "досудебный порядок обязателен".
   - п.4 (обязанность выдать груз) vs п.11 (право удержать груз при неоплате) - при
     буквальном чтении друг другу противоречили (п.4 без оговорок обязывал выдать, п.11
     разрешал не выдавать). Юридически разрешилось бы через "специальная норма важнее
     общей", но добавлена явная оговорка в п.4 ("за исключением случаев... предусмотренных
     п.11"), чтобы не оставлять почву для спора о толковании.
   - п.6 (непредоставление груза) - убрана перекрёстная ссылка на п.7 (там речь про ДРУГОЙ
     триггер - отказ ДО подачи машины, а не неявка груза ПОСЛЕ 24 часов ожидания); сумма 20%
     осталась той же, но записана в п.6 прямо, без заимствования чужого условия по номеру.
     Заодно термин приведён к единому "прибытие на адрес погрузки" (было "постановка под
     погрузку" - то же по смыслу, что и в п.10, но другими словами, лишний повод для спора
     о том, один это момент времени или нет).
   - п.8 (превышение габаритов) - "может изменить провозной тариф" было без критерия;
     добавлено "по действующим расценкам ИСПОЛНИТЕЛЯ", чтобы это не читалось как
     ничем не ограниченное право менять цену на любую цифру.
   - ОСОЗНАННО НЕ ТРОНУТО (риск, не баг): п.6 суммирует простой (по ставке п.5) И штраф
     20% (сумма п.7) при срыве погрузки - при большом простое (сутки+) комбинация может
     превысить саму стоимость рейса. Это разные виды потерь (простой - цена стоящей машины,
     20% - цена сорванного рейса), поэтому суммирование юридически обосновано, НО при
     оспаривании в суде размер неустойки можно попросить снизить по ст.333 ГК РФ как
     несоразмерный - Влад видел эту цифру раньше и явно попросил суммировать, а не выбирать
     одно из двух, поэтому оставлено как есть, но это открытый риск, не гарантия.
   - п.2 (акцепт), пп.(б) "предоставление данных для перевозки" - слабее трёх остальных
     оснований (оплата/допуск к погрузке/подпись документов): в реальном рабочем процессе
     дашборда данные для перевозки обычно вводятся МЕНЕДЖЕРОМ ДО того, как договор-заявка
     вообще сформирована и отправлена заказчику - то есть по времени это может быть ДО
     оферты, а не акцепт НА оферту. Формулировку менять не стал (само основание не вредит,
     раз есть остальные три), но полагаться в споре лучше на (а)/(в)/(г), не на (б). */
/* 21.09 (вечер), РЕДАКЦИЯ 2 - полная пересборка текста после синтеза с разбором ChatGPT
   (ТЗ Влада contract_request_context_for_claude_code.md) и проверки по первоисточникам -
   ст. 8/10/11/12/34/35/38/40 УАТ, ст. 328/359-360/403/435/438/790/796 ГК. Проект текста,
   таблица «было/стало/почему», проверка противоречий - plans/2026-09-21-contract-request-
   v2-legal-draft.md, там же решения Влада (2 ч на погрузку + 2 ч на выгрузку вместо «8 ч
   суммарно»; срок доставки и режим оплаты выбираются в конструкторе; 1%/сутки за нашу
   просрочку с потолком 10%; пени 0,1%; номер документа = o.id). Два пункта ДИНАМИЧЕСКИЕ и
   помечены маркерами, а не индексами (раньше был хардкод specialTerms[4] - ломался при любой
   вставке пункта выше): '{IDLE}' -> idleTermText_(ставка по тоннажу), '{PAYMENT}' ->
...
var CONTRACT_SPECIAL_TERMS_ = [
  ...
  /* 5 */ 'Погрузка, размещение и крепление груза на транспортном средстве, а также выгрузка осуществляются силами и средствами ЗАКАЗЧИКА (грузоотправителя/грузополучателя) (ч. 8 ст. 11 УАТ) с соблюдением требований безопасности, нормативных весовых и габаритных параметров и сохранности транспортного средства. ЗАКАЗЧИК обеспечивает: подготовку груза к перевозке (ст. 10 УАТ); передачу водителю всех необходимых документов и сведений; соответствие фактических массы, количества и состояния груза исходным характеристикам и товаросопроводительным документам; возможность присутствия водителя при погрузке и проверки соответствия груза. Указания водителя по безопасности размещения и крепления груза обязательны для грузоотправителя. При выявлении несоответствия исходным характеристикам, небезопасного размещения или крепления, отсутствия документов либо иных обстоятельств, создающих угрозу безопасности движения, сохранности груза, транспортного средства или третьих лиц, ИСПОЛНИТЕЛЬ вправе не начинать движение до их устранения. Время ожидания устранения учитывается как время погрузки/выгрузки по п. 7; отказ ИСПОЛНИТЕЛЯ начать движение по указанным основаниям не является отказом от перевозки по п. 9 и срывом по вине ИСПОЛНИТЕЛЯ. Если устранение невозможно либо ЗАКАЗЧИК от него отказывается, груз считается непредъявленным (ч. 4 ст. 10 УАТ) и применяется п. 10 - при несоответствии характеристик, в остальных случаях - подпункт «б» п. 8.',
  /* 6 */ 'ИСПОЛНИТЕЛЬ обязан подать под погрузку технически исправное транспортное средство, пригодное для перевозки груза с исходными характеристиками, в согласованное время; доставить груз в пункт выгрузки и выдать его грузополучателю, указанному в товаросопроводительных документах, с получением отметок о выдаче - за исключением случаев правомерного приостановления исполнения и удержания груза (п. 13). Адреса погрузки и выгрузки не являются указанием ЗАКАЗЧИКА на конкретный маршрут: маршрут движения определяется ИСПОЛНИТЕЛЕМ с учётом характеристик транспортного средства и груза, дорожных ограничений, требований безопасности и условий разрешительных документов, если иной маршрут прямо не согласован СТОРОНАМИ. Согласованный срок доставки определён исходя из исходных характеристик, режима труда и отдыха водителя и условий движения; он продлевается на время задержек, вызванных обстоятельствами на стороне ЗАКАЗЧИКА (пп. 5, 7, 8, 10), включая вызванное такими задержками время обязательного отдыха водителя, а также на время действий органов контроля, ограничений движения и погодных условий, не вызванных нарушением со стороны ИСПОЛНИТЕЛЯ, о чём ИСПОЛНИТЕЛЬ уведомляет ЗАКАЗЧИКА по каналам п. 1. За просрочку доставки по вине ИСПОЛНИТЕЛЯ ЗАКАЗЧИК вправе требовать штраф в размере 1% стоимости перевозки за каждые полные сутки просрочки, но не более 10% стоимости перевозки (ч. 11 ст. 34 УАТ).',
  /* 7 */ '{IDLE}',
  /* 8 */ 'Если по истечении нормативного времени погрузки (2 часа, п. 7) с момента прибытия транспортного средства на адрес погрузки груз не предоставлен, ИСПОЛНИТЕЛЬ вправе по своему усмотрению: (а) продолжить ожидание - в этом случае ЗАКАЗЧИК уплачивает штраф за простой по п. 7 за каждый начатый час ожидания сверх нормативного времени; либо (б) в любой момент после истечения нормативного времени покинуть адрес погрузки, уведомив ЗАКАЗЧИКА по каналам п. 1 - в этом случае перевозка признаётся сорванной по вине ЗАКАЗЧИКА, и ЗАКАЗЧИК уплачивает штраф за простой, начисленный до момента отъезда по подпункту «а», а также, дополнительно к нему, штраф в размере 20% стоимости перевозки (ч. 1 ст. 35 УАТ). Решение о продолжении либо прекращении ожидания ИСПОЛНИТЕЛЬ принимает самостоятельно с учётом фактических обстоятельств и без дополнительного согласования с ЗАКАЗЧИКОМ. Если по истечении 24 часов с момента прибытия груз так и не предоставлен либо ЗАКАЗЧИК/грузоотправитель уведомил ИСПОЛНИТЕЛЯ об отсутствии груза, перевозка признаётся сорванной на условиях подпункта «б» вне зависимости от решения ИСПОЛНИТЕЛЯ.',
  /* 9 */ 'За отказ от перевозки менее чем за 24 часа до согласованного времени подачи транспортного средства виновная СТОРОНА уплачивает другой СТОРОНЕ штраф в размере 20% стоимости перевозки. Отказ ЗАКАЗЧИКА после прибытия транспортного средства на адрес погрузки влечёт, помимо штрафа, оплату простоя по п. 7 (без нормативного времени, с момента прибытия) и возмещение расходов по п. 11. Отказ ИСПОЛНИТЕЛЯ от перевозки либо неподача транспортного средства, вызванные обстоятельствами на стороне ЗАКАЗЧИКА (пп. 5, 8, 10, 12, 14), ответственности ИСПОЛНИТЕЛЯ не влекут. Стоимость перевозки для расчёта штрафов - сумма, указанная в разделе «Оплата».',
  /* 10 */ 'Если фактические масса, габариты, осевые нагрузки, состояние или иные характеристики груза не соответствуют исходным (п. 4) - в том числе когда это выявлено после начала движения, при весовом или габаритном контроле: (а) ИСПОЛНИТЕЛЬ вправе приостановить погрузку либо движение до определения безопасного и соответствующего закону способа продолжения перевозки, а при невозможности - отказаться от перевозки, что признаётся срывом по вине ЗАКАЗЧИКА; (б) ЗАКАЗЧИК уплачивает ИСПОЛНИТЕЛЮ штраф в размере 20% стоимости перевозки (ч. 3 ст. 35 УАТ) и, независимо от штрафа, возмещает все документально подтверждённые дополнительные расходы и убытки ИСПОЛНИТЕЛЯ, вызванные несоответствием: изменение маршрута и дополнительный пробег, получение или переоформление специальных разрешений, дополнительное сопровождение и машины прикрытия, привлечение дополнительных или иных транспортных средств, платные дороги и стоянки, административные штрафы и иные платежи, возложенные на ИСПОЛНИТЕЛЯ вследствие несоответствия, простой по п. 7 без нормативного времени с момента приостановки; (в) при продолжении перевозки стоимость перевозки определяется исходя из фактических характеристик по действующим расценкам ИСПОЛНИТЕЛЯ, разница оплачивается дополнительно. Уплата штрафа не освобождает ЗАКАЗЧИКА от возмещения причинённого ущерба (ч. 3 ст. 35 УАТ).',
  /* 11 */ 'Расходы, не включённые в стоимость перевозки и вызванные обстоятельствами на стороне ЗАКАЗЧИКА либо его дополнительными требованиями (изменение адреса или времени, дополнительный заезд, ожидание документов, переадресовка, сопровождение сверх указанного в договор-заявке, платные дороги и стоянки по требованию ЗАКАЗЧИКА и т.п.), возмещаются ЗАКАЗЧИКОМ сверх стоимости перевозки на основании счёта ИСПОЛНИТЕЛЯ с приложением подтверждающих документов в течение 3 (трёх) рабочих дней с даты получения счёта по каналам п. 1.',
  /* 12 */ '{PAYMENT}',
  /* 13 */ 'При непоступлении оплаты в срок, установленный п. 12 для режимов предоплаты и оплаты на погрузке, ИСПОЛНИТЕЛЬ на основании п. 2 ст. 328 ГК РФ вправе приостановить исполнение и не приступать к подаче транспортного средства либо к погрузке до зачисления оплаты. При непоступлении оплаты в срок, установленный п. 12 для режима оплаты на выгрузке, а равно при неоплате причитающихся по настоящей договор-заявке платежей за перевозку, ИСПОЛНИТЕЛЬ вправе приостановить выдачу груза и удерживать находящийся у него груз до исполнения ЗАКАЗЧИКОМ обязательства по оплате перевозки и иных причитающихся ИСПОЛНИТЕЛЮ платежей по перевозке в порядке и на основаниях, предусмотренных законодательством РФ (п. 4 ст. 790, ст. 359-360 ГК РФ). Об удержании ИСПОЛНИТЕЛЬ уведомляет ЗАКАЗЧИКА по каналам п. 1. За период удержания начисляется простой по п. 7 без нормативного времени, а расходы на стоянку и хранение груза возмещает ЗАКАЗЧИК. Обязанность ЗАКАЗЧИКА заблаговременно уведомить грузополучателя об условиях оплаты и о праве удержания установлена п. 3.',
  /* 14 */ 'Любое изменение исходных характеристик, адресов, времени подачи, срока доставки, типа транспортного средства, сопровождения, стоимости или порядка оплаты после формирования настоящей редакции подлежит согласованию по каналам п. 1 и оформляется новой редакцией договор-заявки с новым номером редакции; предыдущая редакция сохраняется. До согласования новой редакции действует настоящая. Изменение, заявленное до подачи транспортного средства, может повлечь пересчёт стоимости; если перевозка с изменёнными параметрами не может быть выполнена назначенным транспортным средством, применяются последствия отказа ЗАКАЗЧИКА по п. 9. Изменение характеристик, выявленное или заявленное после подачи транспортного средства, регулируется п. 10.',
  /* 15 */ 'ИСПОЛНИТЕЛЬ отвечает за сохранность груза с момента принятия к перевозке до выдачи в размере и на условиях ст. 796 ГК РФ и ст. 34 УАТ и освобождается от ответственности, если утрата, недостача или повреждение произошли вследствие обстоятельств, которые он не мог предотвратить и устранение которых от него не зависело (ст. 36 УАТ), в том числе вследствие ненадлежащих упаковки, размещения или крепления груза, недостоверных исходных характеристик, скрытых дефектов груза. Груз принимается и выдаётся по наружному осмотру без проверки внутреннего состояния, комплектности и работоспособности техники. Повреждение или недостача фиксируются при выдаче отметкой в транспортной накладной и актом с участием водителя (ст. 38 УАТ); при отсутствии таких отметок груз считается выданным в надлежащем состоянии, если ЗАКАЗЧИКОМ не доказано иное. Стоимость груза для целей ответственности подтверждается документами, представленными ЗАКАЗЧИКОМ до начала перевозки. Страхование груза ИСПОЛНИТЕЛЕМ не осуществляется, если это прямо не указано в настоящей договор-заявке.',
  /* 16 */ 'Досудебный (претензионный) порядок урегулирования споров обязателен. Претензия направляется по каналам п. 1 либо почтой по адресу СТОРОНЫ, указанному в настоящей договор-заявке. СТОРОНА, получившая претензию, обязана рассмотреть её и направить заявителю мотивированный ответ в течение 30 (тридцати) дней со дня получения претензии (ст. 40 УАТ). Споры рассматриваются в арбитражном суде по месту нахождения ИСПОЛНИТЕЛЯ, если иная подсудность не установлена императивными нормами законодательства РФ.'
];
...
/* Карточка заказчика для шапки договора (Влад 21.09) - только если заявка привязана к
   справочнику по ИНН (customer_entity_id, тот же механизм, что уже подставляет поле
   «Заказчик»: подсказка по истории или явный поиск по ИНН + «Сохранить в справочник и
   подставить»). Без привязки - реквизиты взять неоткуда (ни справочник, ни ДаДата/Чеко -
   их вызывают ПО ИНН, а ИНН просто не вводили), поэтому genContractPdf блокирует выгрузку
   до привязки, а не идёт в отдельный live-поиск здесь. */
function fetchCustomerEntity_(o) {
  if (!o.customer_entity_id) return Promise.resolve(null);
  return apiGet('/orders/customer_entity', { id: o.customer_entity_id }).then(function (r) {
    return (r && r.ok && r.data && r.data.ok) ? r.data.entity : null;
  }).catch(function () { return null; });
}
/* Тип техники (текст из справочника «Задание») -> код vehType калькулятора, для расчёта
   базовой ставки смены при штрафе за простой (см. CONTRACT_IDLE_OP_HOURS_ ниже). Тент и
   Машина прикрытия - решение Влада 21.09 (план
   plans/2026-09-21-contract-idle-time-tonnage-gate.md): Тент считается как Длинномер,
   Машина прикрытия - отдельная фиксированная ставка (не по тоннажу - это сопровождение,
   груз не везёт), уже есть в калькуляторе как отдельная константа. */
var CONTRACT_VEH_TYPE_MAP_ = {
  'трал': 'tral', 'длинномер': 'board', 'тент': 'board', 'панелевоз': 'panel',
  'трал под кран': 'tral_crane', 'трал-корыто': 'korito', 'машина прикрытия': 'escort'
};
function contractVehType_(equipmentType) {
  var key = String(equipmentType || '').trim().toLowerCase();
  if (CONTRACT_VEH_TYPE_MAP_[key]) return CONTRACT_VEH_TYPE_MAP_[key];
  if (/faymonville/i.test(key)) return 'faymon'; // "Faymonville (60+ т)" и варианты записи
  return null;
}
/* Простой (Особые условия п.7) - Влад 21.09 (вечер): нормативное время 2 часа на погрузку и
   2 часа на выгрузку, КАЖДАЯ операция отдельно от своего прибытия. Было «8 ч суммарно на
   рейс (7 работы + 1 подача)» - путало на дальних рейсах («как уложиться в 8 часов, если
...
function fetchIdleRates_(o) {
  var vt = contractVehType_(o.equipment_type);
  var w = num(o.cargo_weight_t);
  if (!vt) return Promise.resolve({ error: 'тип техники «' + (o.equipment_type || '') + '» не привязан к тарифу калькулятора' });
...
/* Текст п.7 «Простой» - единственное место, где в договор попадают цифры ставки
   (idleRates: hourly/dayRate из /calculator/shift_rate по тоннажу этого груза). Формулировка
   согласована 21.09 (вечер), см. plans/2026-09-21-contract-request-v2-legal-draft.md. */
var RU_HOURS_WORD_ = { 1: 'один', 2: 'два', 3: 'три', 4: 'четыре' };
function idleOpHoursText_() { return CONTRACT_IDLE_OP_HOURS_ + ' (' + (RU_HOURS_WORD_[CONTRACT_IDLE_OP_HOURS_] || CONTRACT_IDLE_OP_HOURS_) + ') ' + (CONTRACT_IDLE_OP_HOURS_ === 1 ? 'час' : 'часа'); }
function idleTermText_(rates) {
  var hourly = Math.round(rates.hourly), dayRate = Math.round(rates.dayRate);
  return 'Нормативное время погрузки составляет ' + idleOpHoursText_() + ' с момента прибытия транспортного средства на адрес погрузки, но не ранее согласованного времени подачи, указанного в разделе «РЕЙС»; нормативное время выгрузки - ' + idleOpHoursText_() + ' с момента фактического прибытия транспортного средства на адрес выгрузки. Нормативное время предоставляется на каждую операцию отдельно; время движения транспортного средства между пунктами погрузки и выгрузки учтено в стоимости перевозки и на нормативное время операций не влияет. Операция считается оконченной, когда транспортное средство освобождено и водителю переданы оформленные документы. По истечении нормативного времени транспортное средство считается находящимся в простое по причинам, зависящим от ЗАКАЗЧИКА, и ЗАКАЗЧИК уплачивает ИСПОЛНИТЕЛЮ штраф (ч. 4 ст. 35 УАТ) в размере ' + fmtP(hourly) + ' за каждый начатый час простоя в пределах первых суток простоя и ' + fmtP(dayRate) + ' за каждые последующие начатые сутки (из расчёта ' + CONTRACT_IDLE_DAY_HOURS_ + ' часов простоя в сутки по указанной часовой ставке). Ставка рассчитана от базовой ставки смены транспортного средства данного типа для груза данной массы. Простой по основаниям пп. 8, 10, 12 и 13 оплачивается по указанной ставке с момента, определённого соответствующим пунктом, без предоставления нормативного времени. Время прибытия, начала и окончания простоя фиксируется отметками в транспортной накладной и (или) актом о простое (ст. 38 УАТ), а также может подтверждаться перепиской по каналам п. 1, данными спутникового мониторинга (ГЛОНАСС/GPS) транспортного средства, фото- и видеофиксацией с отображением времени, данными информационной системы ИСПОЛНИТЕЛЯ и иными доказательствами. Отказ или уклонение представителя ЗАКАЗЧИКА, грузоотправителя или грузополучателя от подписания акта о простое либо от проставления отметок не освобождает ЗАКАЗЧИКА от оплаты простоя, если его наличие и продолжительность подтверждаются иными указанными доказательствами; в этом случае водитель вправе сделать в перевозочных документах отметку об отказе с указанием причины и составить акт в одностороннем порядке.';
}
/* Режимы оплаты (Особые условия п.12) - выбираются в конструкторе договора (Влад 21.09,
   утверждённое превью: артефакт «Конструктор договора-заявки»). Ключи = enum
   plan_orders.payment_mode на сервере, payment_days (1..7) только для postpay_docs. */
var CONTRACT_PAY_MODES_ = {
  prepay: { label: 'Предоплата 100%', badge: 'без риска', risk: false, desc: 'До подачи транспорта. Машина выезжает только после зачисления денег на счёт.' },
  postpay_docs: { label: 'Оплата по документам', badge: 'по УПД', risk: false, desc: 'Срок считается с даты получения УПД (не подписания). Молчание 3 рабочих дня = услуги приняты. Пени 0,1% в день.' },
  on_loading: { label: 'Оплата на погрузке', badge: '1 час с прибытия', risk: true, desc: 'Час считается с момента прибытия машины на адрес погрузки. Без оплаты - не грузимся.', warn: 'Не пришла за час - платный простой (п. 7) до зачисления; после 2 часов можем уехать со срывом (п. 8).' },
  on_unloading: { label: 'Оплата на выгрузке', badge: '1 час с прибытия', risk: true, desc: 'Час считается с момента прибытия машины на адрес выгрузки. Без оплаты - груз не выдаём.', warn: 'Не пришла за час - платный простой (п. 7) и удержание груза на трале (п. 13) до зачисления.' }
};
var CONTRACT_PAY_MODE_ORDER_ = ['prepay', 'postpay_docs', 'on_loading', 'on_unloading'];
var RU_NUM_GEN_ = { 1: 'одного', 2: 'двух', 3: 'трёх', 4: 'четырёх', 5: 'пяти', 6: 'шести', 7: 'семи' };
function ruWorkDays_(n) { return n === 1 ? 'рабочего дня' : 'рабочих дней'; }
/* Короткая строка режима - плашка «ОПЛАТА» на первой странице PDF, предпросмотр в
   конструкторе, карточка заявки. Цифры/смысл те же, что в полном п.12. */
function payModeShort_(mode, days) {
  if (mode === 'postpay_docs') return 'оплата в течение ' + days + ' ' + ruWorkDays_(days) + ' с даты получения УПД';
  if (mode === 'on_loading') return 'оплата в течение 1 часа с момента прибытия на адрес погрузки';
  if (mode === 'on_unloading') return 'оплата в течение 1 часа с момента прибытия на адрес выгрузки';
  return 'предоплата 100% до подачи транспортного средства';
}
function paymentTermText_(mode, days) {
  var m;
  if (mode === 'postpay_docs') {
    m = 'оплата по документам - в течение ' + days + ' (' + (RU_NUM_GEN_[days] || days) + ') ' + ruWorkDays_(days) + ' с даты получения ЗАКАЗЧИКОМ акта/УПД по каналам п. 1 (акт/УПД направляется после выгрузки). Услуги считаются принятыми, если в течение 3 (трёх) рабочих дней с даты получения акта/УПД ЗАКАЗЧИК не направил мотивированных возражений; неподписание акта/УПД не продлевает срок оплаты.';
  } else if (mode === 'on_loading') {
    m = 'оплата на погрузке - в течение 1 (одного) часа с момента прибытия транспортного средства на адрес погрузки, указанный в разделе «РЕЙС». До зачисления оплаты ИСПОЛНИТЕЛЬ вправе не приступать к погрузке (п. 13); с момента истечения указанного часа и до зачисления начисляется простой по ставке п. 7 без нормативного времени; по истечении 2 (двух) часов с момента прибытия ИСПОЛНИТЕЛЬ вправе по своему усмотрению продолжить ожидание либо покинуть адрес погрузки с последствиями подпункта «б» п. 8.';
  } else if (mode === 'on_unloading') {
    m = 'оплата на выгрузке - в течение 1 (одного) часа с момента прибытия транспортного средства на адрес выгрузки, указанный в разделе «РЕЙС». До зачисления оплаты ИСПОЛНИТЕЛЬ вправе не приступать к выгрузке и удерживать груз (п. 13); с момента истечения указанного часа и до зачисления начисляется простой по ставке п. 7 без нормативного времени.';
  } else {
    m = 'предоплата 100% - до подачи транспортного средства. Обязанность ИСПОЛНИТЕЛЯ подать транспортное средство возникает после зачисления оплаты; при непоступлении оплаты к согласованному времени подачи ИСПОЛНИТЕЛЬ вправе не подавать транспортное средство либо перенести подачу, что не является отказом от перевозки по п. 9.';
  }
...
/* ═══ Конструктор договора-заявки (Влад 21.09: «диалог - это конструктор, где ВЫБИРАЕМ
   динамические данные») - модальное окно поверх drawer. Здесь только то, что выбирается на
   каждый рейс: срок доставки (дата + время прибытия на выгрузку) и режим оплаты (+ дни для
   постоплаты). Всё остальное (груз, адреса, машина, реквизиты, ставка простоя) уже в заявке
   и справочнике. Кнопка «Сформировать» неактивна, пока срок не выбран или он раньше времени
   загрузки. DOM создаётся один раз при первом открытии и переиспользуется. */
var ccState_ = null;
function ccEnsureDom_() {
  if ($('#op2-cc')) return;
  var scrim = document.createElement('div'); scrim.className = 'op2-cc-scrim'; scrim.id = 'op2-cc-scrim';
  var box = document.createElement('div'); box.className = 'op2-cc'; box.id = 'op2-cc';
  box.setAttribute('role', 'dialog'); box.setAttribute('aria-labelledby', 'op2-cc-title');
  box.innerHTML =
    '<div class="op2-cc-hd"><h3 id="op2-cc-title">Конструктор договора-заявки</h3><span class="op2-sub" id="op2-cc-sub"></span></div>' +
    '<div class="op2-cc-body" id="op2-cc-body"></div>' +
    '<div class="op2-cc-ft"><span class="op2-meta" id="op2-cc-meta"></span>' +
      '<button type="button" class="op2-ghost" id="op2-cc-cancel">Отмена</button>' +
      '<button type="button" class="op2-dbtn op2-primary" id="op2-cc-ok">Сформировать договор</button></div>';
  document.body.appendChild(scrim); document.body.appendChild(box);
  scrim.addEventListener('click', ccClose_);
  $('#op2-cc-cancel').addEventListener('click', ccClose_);
  $('#op2-cc-ok').addEventListener('click', function () {
    var st = ccState_;
    if (!st || !st.valid) return;
    ccClose_();
    st.onConfirm(st.sel());
  });
  document.addEventListener('keydown', function (e) { if (e.key === 'Escape' && ccState_) ccClose_(); });
}
function ccClose_() {
  ccState_ = null;
  var b = $('#op2-cc'), s = $('#op2-cc-scrim');
  if (b) b.classList.remove('op2-open');
  if (s) s.classList.remove('op2-open');
}
function openContractConstructor_(o, ent, idle, onConfirm) {
  ccEnsureDom_();
  var loadTime = o.service_time ? String(o.service_time).slice(0, 5) : '';
  var st = {
    o: o, onConfirm: onConfirm, valid: false,
    date: o.delivery_date || o.service_date || '',
  ...
  $('#op2-cc-meta').textContent = 'Заявка №' + oNo(o) + ' · id ' + o.id + ' · ' + (ent.short || ent.name || '');
  var opts = CONTRACT_PAY_MODE_ORDER_.map(function (k) {
    var m = CONTRACT_PAY_MODES_[k];
    return '<button type="button" class="op2-cc-opt" data-mode="' + k + '" role="radio">' +
      '<span class="op2-cc-radio"></span>' +
      '<span class="op2-row1"><span class="op2-ttl">' + esc(m.label) + '</span><span class="op2-badge ' + (m.risk ? 'op2-risk' : 'op2-safe') + '">' + esc(m.badge) + '</span></span>' +
      '<span class="op2-desc">' + esc(m.desc) + '</span>' +
      (k === 'postpay_docs'
        ? '<span class="op2-cc-step"><span class="op2-lbl">Срок оплаты:</span><span class="op2-ctl"><button type="button" data-step="-1" aria-label="Меньше">−</button><span class="op2-nn" id="op2-cc-days"></span><button type="button" data-step="1" aria-label="Больше">+</button></span><span class="op2-cap">рабочих дней (макс. 7)</span></span>'
        : '') +
      (m.warn ? '<span class="op2-cc-warn">' + esc(m.warn) + '</span>' : '') +
    '</button>';
  }).join('');
  var hourly = Math.round(idle.hourly), dayRate = Math.round(idle.dayRate);
  $('#op2-cc-body').innerHTML =
    '<div class="op2-cc-sec"><div class="op2-cc-k"><span class="op2-n">01</span><span class="op2-t">Срок доставки</span><span class="op2-auto">загрузка ' + esc(dmy(o.service_date)) + (loadTime ? ' ' + esc(loadTime) : '') + '</span></div>' +
      '<div class="op2-cc-dl">' +
        '<div class="op2-fld"><label for="op2-cc-date">Дата</label><input id="op2-cc-date" type="date" value="' + esc(st.date) + '"></div>' +
        '<div class="op2-fld"><label for="op2-cc-time">Время прибытия на выгрузку</label><input id="op2-cc-time" type="time" value="' + esc(st.time) + '"></div>' +
  ...
  $('#op2-cc-opts').addEventListener('click', function (e) {
    ...
    var opt = e.target.closest('.op2-cc-opt');
    if (opt) { st.mode = opt.dataset.mode; ccUpdate_(); }
  });
  ccUpdate_();
  $('#op2-cc-scrim').classList.add('op2-open');
  $('#op2-cc').classList.add('op2-open');
  setTimeout(function () { var d = $('#op2-cc-date'); if (d && !st.time) { var t = $('#op2-cc-time'); if (t) t.focus(); } }, 350);
}
function ccUpdate_() {
  var st = ccState_; if (!st) return;
  var o = st.o;
  $$('#op2-cc-opts .op2-cc-opt').forEach(function (el) { var on = el.dataset.mode === st.mode; el.classList.toggle('op2-on', on); el.setAttribute('aria-checked', on ? 'true' : 'false'); });
  var dn = $('#op2-cc-days'); if (dn) dn.textContent = st.days;
  var minus = $('#op2-cc-opts [data-step="-1"]'), plus = $('#op2-cc-opts [data-step="1"]');
  if (minus) minus.disabled = st.days <= 1;
  if (plus) plus.disabled = st.days >= 7;
  var ok = !!st.date && !!st.time;
  var loadTs = o.service_date ? new Date(o.service_date + 'T' + (o.service_time ? String(o.service_time).slice(0, 5) : '00:00') + ':00') : null;
  var dlTs = ok ? new Date(st.date + 'T' + st.time + ':00') : null;
  var bad = !!(ok && loadTs && dlTs < loadTs);
  var hint = $('#op2-cc-dlhint');
  hint.classList.toggle('op2-warn', bad);
  hint.textContent = bad
    ? 'Срок доставки раньше времени загрузки (' + dmy(o.service_date) + (o.service_time ? ' ' + String(o.service_time).slice(0, 5) : '') + ') - так в договор не попадёт.'
    : (ok ? 'Попадает в «РЕЙС» и в п. 6. Просрочка по нашей вине - 1% за сутки, не более 10%; задержки по вине заказчика срок продлевают.' : 'Укажи дату и время прибытия на выгрузку - без них договор не формируется.');
...
/* Снимок всего, что попало в PDF, - уходит в contract_versions на сервере (п.1 и п.14
   договора: «каждая сформированная редакция сохраняется»). Через год можно доказать, какая
   именно редакция и с какими цифрами ушла заказчику. */
function contractSnapshot_(o, ent, custEnt, idle, sel, terms) {
  var mainExec = oOwn(o).filter(function (e) { return e.role === 'main'; })[0] || oOwn(o)[0] || (o.executors || [])[0] || {};
  function ents(e) { return { id: e.id, full_name: e.full_name || e.name, inn: e.inn, kpp: e.kpp, ogrn: e.ogrn, legal_address: e.legal_address, bank_account: e.bank_account, bank_name: e.bank_name, bank_corr_account: e.bank_corr_account, bank_bik: e.bank_bik, director: e.director, director_post: e.director_post }; }
  return {
    order_id: o.id, day_no: o.day_no, service_date: o.service_date, service_time: o.service_time,
    executor: ents(ent), customer: ents(custEnt),
    cargo: o.cargo, cargo_weight_t: o.cargo_weight_t, cargo_dims: o.cargo_dims, equipment_type: o.equipment_type, gabarit: o.gabarit,
    load_address: contractAddrText_(o.load_address), load_contact_name: o.load_contact_name, load_contact_phone: o.load_contact_phone,
    unload_address: contractAddrText_(o.unload_address), unload_contact_name: o.unload_contact_name, unload_contact_phone: o.unload_contact_phone,
...
function genContractPdf(o) {
  ...
  if (!ent.has_bank || !ent.has_stamp) { logUiEvent_('blocked_click', 'contract', 'нет реквизитов/печати: ' + (ent.short || ent.name)); toast('<span class="op2-warn">Конструктор договора</span> · у «' + esc(ent.short || ent.name) + '» не хватает реквизитов/печати в Справочниках - дозаполни там'); return; }
  /* Гейт по данным груза (Влад 21.09) - без наименования/габаритов/массы простой считать
     не из чего, договор не выгружаем. Тот же приём, что и гейт исполнителя выше - тост
     при клике с точной причиной, кнопка не предзапирается заранее. */
  var missing = [];
  if (!o.cargo) missing.push('наименование груза');
  if (!o.cargo_dims) missing.push('габариты груза');
  if (!o.cargo_weight_t) missing.push('масса груза');
  if (missing.length) { logUiEvent_('blocked_click', 'contract', 'не хватает данных груза: ' + missing.join(', ')); toast('<span class="op2-warn">Конструктор договора</span> · заполни в заявке: ' + esc(missing.join(', '))); return; }
  /* Гейт по заказчику (Влад 21.09) - без привязки к справочнику по ИНН реквизиты в шапке
     взять неоткуда, договор не выгружаем. Привязка делается ПРЯМО в поле «Заказчик» -
     ввести ИНН, нажать «Сохранить в справочник и подставить» (уже существующий механизм). */
  if (!o.customer_entity_id) { logUiEvent_('blocked_click', 'contract', 'заказчик не привязан к справочнику'); toast('<span class="op2-warn">Конструктор договора</span> · заказчик не привязан к справочнику - в поле «Заказчик» введи ИНН и подставь из подсказки'); return; }
  toast('Готовим конструктор договора…');
  /* Данные тянем ДО открытия конструктора - в предпросмотре видна реальная ставка простоя
     по тоннажу, а не заглушка; печать/подпись/карточка заказчика к моменту «Сформировать»
     уже на руках. */
  Promise.all([
    fetchEntityStampDataUrl_(ent.id),
    fetchPersonSignatureDataUrl_(ent.signer_person_id),
    fetchIdleRates_(o),
    fetchCustomerEntity_(o)
    ...
    openContractConstructor_(o, ent, idle, function (sel) {
      ...
      apiPostJson('/orders/contract_version', { order_id: o.id, snapshot: snapshot, delivery_date: sel.delivery_date, delivery_time: sel.delivery_time, payment_mode: sel.payment_mode, payment_days: sel.payment_days, correspondence_email: sel.correspondence_email, correspondence_phone: sel.correspondence_phone }).then(function (r) {
        ...
        try { drawContractPdf_(o, ent, results[0], results[1], idle, custEnt, sel, meta, terms); }
        catch (e) { logUiEvent_('save_error', 'contract_pdf', String((e && e.message) || e).slice(0, 100)); toast('Не удалось собрать PDF: ' + (e && e.message || e)); return; }
        S.tickUp();
        loadOrders();
      });
    });
  }).catch(function () { logUiEvent_('save_error', 'contract_stamp_fetch', 'сеть'); toast('Ошибка сети - не удалось получить печать/подпись/ставку простоя/заказчика'); });
}
/* Отрисовка PDF. sel - выбор из конструктора ({delivery_date, delivery_time, payment_mode,
   payment_days}), meta - ответ /orders/contract_version ({version, document_id,
   generated_at_msk, generated_by_name}), terms - готовый массив пунктов (contractTerms_).
   Вёрстка - «уровень 3 / формуляр» (выбор Влада 22.09 из трёх вариантов агентов): тонкая
   рамка по периметру, «паспортные» рамки сторон, факты рейса в таблице с сеткой, итоги
   вплотную к цифрам, особые условия с висячим отступом и линейкой, подписи в парных рамках.
   Объём теперь 3 страницы (Влад 21.09: «не обязательно две, пусть три»). */
function drawContractPdf_(o, ent, stampUrl, signUrl, idleRates, custEnt, sel, meta, terms) {
  var doc = new window.jspdf.jsPDF({ unit: 'mm', format: 'a4' });
  /* ensureKpFonts_ - НЕ глобальная функция, а приватный хелпер внутри IIFE Калькулятора
     (index.html), отдаётся наружу через window.Clc. Раньше здесь стоял вызов "голого"
     ensureKpFonts_ - typeof всегда был не 'function', шрифт молча не подключался, jsPDF
     откатывался на Helvetica, кириллица печаталась побитово (поймано 15.09 по PDF Влада -
     верификация проверяла размер файла, а не сам текст). */
  if (window.Clc && typeof window.Clc.ensureKpFonts_ === 'function') window.Clc.ensureKpFonts_(doc);
  /* doc.setFont() у jsPDF НЕ бросает исключение на незарегистрированном имени - молча
     откатывается на Times. Проверяем по факту наличия 'PTSans' в getFontList(). */
  ...
  sel = sel || {}; meta = meta || {};
  terms = terms || contractTerms_(idleRates, sel.payment_mode || 'prepay', sel.payment_days || 3);

  var M = 15, W = 210, H = 297, CW = W - M * 2, y = M;
  var FRAME_M = 8;           // рамка по периметру страницы
  var CEIL = 275;            // ниже этой линии контент не начинаем - место под двухстрочный подвал
  var FOOTER_RULE_Y = 279.2, FOOTER_TXT_Y = 282.8, FOOTER_TXT2_Y = 286.2;
  var docNo = String(o.id);
  var docDate = (meta.generated_at_msk ? String(meta.generated_at_msk).slice(0, 10) : dmy(o.service_date));
  var docTitle = 'ДОГОВОР-ЗАЯВКА № ' + docNo + (meta.version > 1 ? ' (редакция ' + meta.version + ')' : '');
  ...
  /* Выравнивание по ширине (Влад 22.09: «как в Word - Ctrl+J»). jsPDF-ное align:justify
     раскладывает через межсловный интервал PDF (Tw), а он не действует на TrueType-шрифты с
     Identity-H (наш PT Sans) - поэтому раскладываем сами: слова строки, кроме последней в
     абзаце, расставляем по ширине колонки. Слишком короткая строка (зазор больше трёх
     пробелов) остаётся как есть - иначе «реки» пустоты. Шрифт должен быть уже выставлен. */
  function textJustified(line, x, yy, width, isLast) {
    var words = String(line).split(" ").filter(Boolean);
    if (isLast || words.length < 2) { text(line, x, yy); return; }
    var sum = 0;
    var ws = words.map(function (w) { var wd = doc.getTextWidth(w); sum += wd; return wd; });
    var gap = (width - sum) / (words.length - 1);
    if (gap > doc.getTextWidth(" ") * 3) { text(line, x, yy); return; }
    var cx = x;
    words.forEach(function (w, i) { text(w, cx, yy); cx += ws[i] + gap; });
  }
  function wrappedJustified(s, x, yy, maxW, lh) {
    var lines = doc.splitTextToSize(String(s == null ? "" : s), maxW);
    lines.forEach(function (l, i) { textJustified(l, x, yy + i * lh, maxW, i === lines.length - 1); });
    return yy + lines.length * lh;
  }
  /* При переносе на новую страницу - компактная «шапка-продолжение» (номер + дата):
     страница, оторванная от пачки, всё равно опознаётся. */
  function ensureSpace(need) {
    if (y + need > CEIL) {
      doc.addPage();
      y = M;
      setF(true, 7.8); doc.setTextColor(110);
      text(docTitle + ' от ' + docDate + ' (продолжение)', M, y);
      doc.setTextColor(0);
      y += 2.6;
      hr(y);
      y += 5;
  ...
  /* Раздел-заголовок - один приём (жирный капс + линия) для всех блоков. */
  function sectionHeader(title, minFollow) {
    ensureSpace(9 + (minFollow || 0));
    setF(true, 10.5); doc.setTextColor(15);
    text(title, M, y);
    y += 2.1;
    doc.setDrawColor(15); doc.setLineWidth(.35); doc.line(M, y, M + CW, y);
    y += 2.8;
  }
  ...
  var mainExec = oOwn(o).filter(function (e) { return e.role === 'main'; })[0] || oOwn(o)[0] || (o.executors || [])[0];
  /* НЕ esc() здесь и ниже - это текст jsPDF, не innerHTML: esc() экранирует &/</>/"/' и они
     попали бы в документ буквально как "&amp;". */
  ...
  /* ── Шапка - без логотипа и без хардкода бренда (Влад 22.09: «всё от того юрлица, от
     которого везём» - исполнитель по заявке МЕНЯЕТСЯ). Номер = id заявки (сквозной,
     уникальный; day_no повторяется каждый день - Влад 21.09), дата = дата формирования
     редакции по Москве (доказательственная цепочка п.1), дата рейса - в РЕЙС. */
  setF(true, 16); doc.setTextColor(10);
  text(docTitle, W / 2, y, { align: 'center' }); y += 6.2;
  setF(false, 9.5); doc.setTextColor(100);
  text('от ' + docDate, W / 2, y, { align: 'center' }); y += 5.5;
  doc.setDrawColor(15); doc.setLineWidth(.4); doc.line(M, y, M + CW, y); y += 0.9;
  doc.setDrawColor(15); doc.setLineWidth(.15); doc.line(M, y, M + CW, y); y += 5.5;
  setF(false, 8.3); doc.setTextColor(0);
  var titleLines = doc.splitTextToSize('«Разовая договор-заявка на организацию внутрироссийской автоперевозки грузов»', CW - 20);
  titleLines.forEach(function (l, i) { text(l, W / 2, y + i * 3.8, { align: 'center' }); });
  y += titleLines.length * 3.8 + 4.5;

  /* ── ИСПОЛНИТЕЛЬ / ЗАКАЗЧИК - парные «паспортные» рамки одинаковой высоты. */
  var colW = CW / 2 - 3, GAP = 6;
  var execLines = [
  ...
  if (custEnt.legal_address) custLines.push('Юридический адрес: ' + custEnt.legal_address);
  var custInnKpp = [];
  if (custEnt.inn) custInnKpp.push('ИНН: ' + custEnt.inn);
  if (custEnt.kpp) custInnKpp.push('КПП: ' + custEnt.kpp);
  if (custInnKpp.length) custLines.push(custInnKpp.join('  '));
  if (custEnt.ogrn) custLines.push('ОГРН: ' + custEnt.ogrn);
  if (custEnt.bank_account) {
    custLines.push('р/с: ' + custEnt.bank_account);
  ...
  var execTotalLines = execLines.reduce(function (n, l) { return n + doc.splitTextToSize(l, innerW).length; }, 0);
  var custTotalLines = custLines.reduce(function (n, l) { return n + doc.splitTextToSize(l, innerW).length; }, 0);
  var partyBoxH = PBOX_HEAD + 2.6 + Math.max(execTotalLines, custTotalLines) * PBOX_LH + 2.4;
  ensureSpace(partyBoxH + 5);
  var pbTop = y;
  function partyBox(x, title, lines) {
    doc.setDrawColor(70); doc.setLineWidth(.28);
    doc.rect(x, pbTop, colW, partyBoxH);
    doc.setFillColor(234, 234, 234);
    doc.rect(x, pbTop, colW, PBOX_HEAD, 'F');
    doc.setDrawColor(70); doc.setLineWidth(.28); doc.line(x, pbTop + PBOX_HEAD, x + colW, pbTop + PBOX_HEAD);
    setF(true, 8.6); doc.setTextColor(15);
    text(title, x + PBOX_PAD, pbTop + PBOX_HEAD - 2.1);
    setF(false, 7.7); doc.setTextColor(20);
    var ty = pbTop + PBOX_HEAD + 3.2;
    lines.forEach(function (l) { ty = wrapped(l, x + PBOX_PAD, ty, innerW, PBOX_LH); });
  }
  partyBox(M, 'ИСПОЛНИТЕЛЬ', execLines);
  partyBox(M + colW + GAP, 'ЗАКАЗЧИК', custLines);
  doc.setTextColor(0);
  y = pbTop + partyBoxH + 6;

  /* ═══ РЕЙС - факты рейса в таблице с сеткой; строка либо целиком на странице, либо целиком
     на следующей (высоты меряются до отрисовки). */
  var FT_PAD_X = 3, FT_PAD_TOP = 2.1, FT_LBL_GAP = 4.0, FT_VAL_LH = 3.9, FT_PAD_BOT = 1.8;
  function measureTableRows(rows) {
    /* Меряем ТЕМ ЖЕ шрифтом, каким рисуется значение (жирный 9) - иначе высота строки
       зависит от того, какой шрифт случайно остался активным (поймано рендером 22.09:
       после подписи-оговорки 6.8 строка водителя недомерялась и наезжала на следующую). */
    setF(true, 9);
    return rows.map(function (cells) {
      var maxLines = 1;
      cells.forEach(function (c) {
        var n = doc.splitTextToSize(c.value || 'уточняется', c.w - FT_PAD_X * 2).length;
        if (n > maxLines) maxLines = n;
      });
      return { maxLines: maxLines, h: FT_PAD_TOP + FT_LBL_GAP + (maxLines - 1) * FT_VAL_LH + FT_PAD_BOT };
    });
  }
  function factTable(rows) {
    var heights = measureTableRows(rows);
    var totalH = heights.reduce(function (s, r) { return s + r.h; }, 0);
    ensureSpace(totalH + 6);
    var top = y;
    doc.setDrawColor(70); doc.setLineWidth(.28);
    doc.rect(M, top, CW, totalH);
    var rowY = top;
    rows.forEach(function (cells, ri) {
      var rh = heights[ri].h;
      var cx = M;
      cells.forEach(function (c, ci) {
        if (ci > 0) { doc.setDrawColor(70); doc.setLineWidth(.2); doc.line(cx, rowY, cx, rowY + rh); }
        cx += c.w;
      });
      cells.forEach(function (c) {
        setF(false, 6.3); doc.setTextColor(115);
        text(c.label, c.x + FT_PAD_X, rowY + FT_PAD_TOP);
        setF(true, 9); doc.setTextColor(12);
        var lines = doc.splitTextToSize(c.value || 'уточняется', c.w - FT_PAD_X * 2);
        lines.forEach(function (l, li) { text(l, c.x + FT_PAD_X, rowY + FT_PAD_TOP + FT_LBL_GAP + li * FT_VAL_LH); });
      });
      rowY += rh;
      if (ri < rows.length - 1) { doc.setDrawColor(70); doc.setLineWidth(.2); doc.line(M, rowY, M + CW, rowY); }
    });
    doc.setTextColor(0);
    y = top + totalH + 5.5;
  }
  var col3 = CW / 3, col2 = CW / 2;
  var deliveryText = sel.delivery_date ? dmy(sel.delivery_date) + (sel.delivery_time ? ' ' + String(sel.delivery_time).slice(0, 5) : '') : (o.delivery_date ? dmy(o.delivery_date) + (o.delivery_time ? ' ' + String(o.delivery_time).slice(0, 5) : '') : 'уточняется');
  var factRows = [
    [
      { x: M, w: col3, label: 'ГРУЗ', value: o.cargo },
      { x: M + col3, w: col3, label: 'МАССА', value: o.cargo_weight_t ? o.cargo_weight_t + ' т' : '' },
      { x: M + col3 * 2, w: col3, label: 'ГАБАРИТЫ (Д×Ш×В)', value: o.cargo_dims }
    ],
    [{ x: M, w: CW, label: 'ОТКУДА - АДРЕС ПОГРУЗКИ', value: contractAddrText_(o.load_address) }],
    [
      { x: M, w: col2, label: 'ДАТА И ВРЕМЯ ПОДАЧИ ПОД ЗАГРУЗКУ', value: dmy(o.service_date) + ' ' + (o.service_time ? String(o.service_time).slice(0, 5) : '') },
      { x: M + col2, w: col2, label: 'КОНТАКТ НА ПОГРУЗКЕ', value: [o.load_contact_name, fmtPhone(o.load_contact_phone)].filter(Boolean).join(' · ') }
    ],
    [{ x: M, w: CW, label: 'КУДА - АДРЕС ВЫГРУЗКИ', value: contractAddrText_(o.unload_address) }],
    [
      { x: M, w: col2, label: 'СОГЛАСОВАННЫЙ СРОК ДОСТАВКИ (ПРИБЫТИЕ НА ВЫГРУЗКУ)', value: deliveryText },
      { x: M + col2, w: col2, label: 'КОНТАКТ НА ВЫГРУЗКЕ', value: [o.unload_contact_name, fmtPhone(o.unload_contact_phone)].filter(Boolean).join(' · ') }
    ],
    [
      { x: M, w: col2, label: 'ТИП И ПАРАМЕТРЫ ПОДВИЖНОГО СОСТАВА', value: (o.equipment_type || '') + (o.gabarit ? ', ' + o.gabarit : '') },
      { x: M + col2, w: col2, label: 'АВТОПОЕЗД И ВОДИТЕЛЬ', value: vehicleLine }
    ]
  ];
  if (o.note) factRows.push([{ x: M, w: CW, label: 'ДОПОЛНИТЕЛЬНЫЕ УСЛОВИЯ', value: o.note }]);
  sectionHeader('РЕЙС', measureTableRows(factRows).reduce(function (s, r) { return s + r.h; }, 0) + 10);
  /* Подпись-оговорка под заголовком: термин «исходные характеристики» из п.4 должен быть
     виден рядом с самими цифрами, а не только в тексте условий. */
  setF(false, 6.8); doc.setTextColor(95);
  y = wrapped('Характеристики груза, адреса, контакты и время подачи сообщены ЗАКАЗЧИКОМ и являются исходными характеристиками (п. 4 Особых условий).', M, y, CW, 3.3) + 1.2;
  doc.setTextColor(0);
  factTable(factRows);
  y += 1;

  /* ── ОПЛАТА - акцентная плашка: сумма + режим из конструктора, текст бьётся с п.12. */
  var paySummary = (fmtP(o.price) || 'уточняется') + (o.cash ? ', наличные' : '') + ' · ' + payModeShort_(sel.payment_mode || 'prepay', sel.payment_days || 3);
  setF(true, 12);
  var payLines = doc.splitTextToSize(paySummary, CW - 10);
  var payH = 4.8 + payLines.length * 4.8 + 2.2;
  ensureSpace(payH + 4);
  doc.setDrawColor(70); doc.setLineWidth(.28);
  doc.setFillColor(240, 240, 240);
  doc.rect(M, y, CW, payH, 'FD');
  setF(false, 6.6); doc.setTextColor(105); text('ОПЛАТА', M + 5, y + 5.0);
  doc.setTextColor(10); setF(true, 12);
  payLines.forEach(function (l, i) { text(l, M + 5, y + 10.7 + i * 4.8); });
  y += payH + 6;
  doc.setTextColor(0);

  /* ── Стоимость - табличная позиция + бордюрная таблица итогов (подпись и цифра в одной
     ячейке - Влад 22.09: «почему так далеко от цифр»). */
  var qtyW = 20, sumW = 42, nameW = CW - qtyW - sumW;
  var svcTitle = 'Транспортные услуги ' + contractEqGenitive_(o.equipment_type) + ' от ' + dmy(o.service_date);
  var svcLines = doc.splitTextToSize(svcTitle, nameW - 6);
  var headH = 5.0, rowH = Math.max(7.8, 2.4 + svcLines.length * 3.9 + 2.4);
  sectionHeader('СТОИМОСТЬ И УСЛОВИЯ ОПЛАТЫ', headH + rowH + 4);
  ensureSpace(headH + rowH + 4);
  var itTop = y;
  doc.setDrawColor(70); doc.setLineWidth(.28);
  doc.rect(M, itTop, CW, headH + rowH);
  doc.setFillColor(234, 234, 234);
  doc.rect(M, itTop, CW, headH, 'F');
  doc.line(M, itTop + headH, M + CW, itTop + headH);
  doc.line(M + nameW, itTop, M + nameW, itTop + headH + rowH);
  doc.line(M + nameW + qtyW, itTop, M + nameW + qtyW, itTop + headH + rowH);
  setF(true, 6.5); doc.setTextColor(80);
  text('НАИМЕНОВАНИЕ УСЛУГИ', M + 3, itTop + 4.2);
  text('КОЛ-ВО', M + nameW + qtyW / 2, itTop + 4.2, { align: 'center' });
  text('СУММА', M + nameW + qtyW + sumW - 3, itTop + 4.2, { align: 'right' });
  setF(false, 8.3); doc.setTextColor(10);
  var svcY = itTop + headH + (rowH - svcLines.length * 3.9) / 2 + 3.1;
  svcLines.forEach(function (l, i) { text(l, M + 3, svcY + i * 3.9); });
  text('1 шт.', M + nameW + qtyW / 2, itTop + headH + rowH / 2 + 1.2, { align: 'center' });
  setF(true, 8.3);
  text(fmtP(o.price) || '—', M + nameW + qtyW + sumW - 3, itTop + headH + rowH / 2 + 1.2, { align: 'right' });
  doc.setTextColor(0);
  y = itTop + headH + rowH + 4;

  var totalsRows = [
    { label: 'ИТОГО:', value: fmtP(o.price) || '—' },
    { label: 'В т.ч. НДС (22%):', value: fmtP(Math.round(vat)) || '—' },
    { label: 'Всего к оплате:', value: fmtP(o.price) || '—', bold: true }
  ];
  var totRowH = 5.6, totLabelW = CW - 44;
  ensureSpace(totRowH * totalsRows.length + 8);
  var totTop = y;
  doc.setDrawColor(70); doc.setLineWidth(.28);
  doc.rect(M, totTop, CW, totRowH * totalsRows.length);
  doc.line(M + totLabelW, totTop, M + totLabelW, totTop + totRowH * totalsRows.length);
  totalsRows.forEach(function (r, i) {
    var ry = totTop + i * totRowH;
    if (i > 0) { doc.setDrawColor(70); doc.setLineWidth(.2); doc.line(M, ry, M + CW, ry); }
    setF(true, r.bold ? 9.6 : 8.4); doc.setTextColor(r.bold ? 8 : 40);
    text(r.label, M + totLabelW - 3, ry + totRowH / 2 + 1.3, { align: 'right' });
    text(r.value, M + CW - 3, ry + totRowH / 2 + 1.3, { align: 'right' });
  });
  doc.setTextColor(0);
  y = totTop + totRowH * totalsRows.length + 4.5;
  setF(false, 7.6);
  y = wrapped('Всего наименований 1, на сумму ' + (fmtP(o.price) || '—') + '.', M, y, CW, 3.7) + 1.2;
  setF(true, 7.6);
  y = wrapped(num(o.price) ? rurWords_(o.price) : 'сумма не указана', M, y, CW, 3.7) + 5;

  /* ═══ Особые условия - номер прижат к правому краю узкой колонки, вертикальная линейка,
     висячий отступ. Текст - contractTerms_ (16 пунктов, два динамических). */
  ensureSpace(9);
  setF(false, 7.8);
  y = wrappedJustified('Настоящим СТОРОНЫ согласовывают условия по организации автоперевозки габаритных/негабаритных грузов во внутрироссийском сообщении, изложенные выше и в пунктах ниже.', M, y, CW, 3.7) + 2.6;
  var NUM_COL = 6.5, RULE_X = M + NUM_COL, TXT_X = RULE_X + 2.2, TXT_W = CW - NUM_COL - 2.2;
  var CLAUSE_LH = 2.95, CLAUSE_GAP = 1.1;
  setF(false, 7.1);
  var firstClauseLines = doc.splitTextToSize(terms[0], TXT_W);
  sectionHeader('ОСОБЫЕ УСЛОВИЯ', Math.min(firstClauseLines.length, 3) * CLAUSE_LH + CLAUSE_GAP);
  terms.forEach(function (term, i) {
    /* Режем строки ТЕМ ЖЕ шрифтом, каким рисуем (7.1) - у первого пункта после
       заголовка (жирный 10.5) строки выходили заметно короче остальных (рендер 22.09). */
    setF(false, 7.1);
    var lines = doc.splitTextToSize(term, TXT_W);
    /* Длинный пункт может не влезть в остаток страницы целиком - переносим по строкам, но
       номер и линейка рисуются у первой части, продолжение - с тем же отступом. */
    var idx = 0;
    var first = true;
    while (idx < lines.length) {
      var room = CEIL - y;
      var fit = Math.max(1, Math.floor(room / CLAUSE_LH));
      if (fit < 3 && idx === 0) { ensureSpace(CEIL + 1); room = CEIL - y; fit = Math.max(1, Math.floor(room / CLAUSE_LH)); }
      var chunk = lines.slice(idx, idx + fit);
      var blockH = chunk.length * CLAUSE_LH;
      if (first) { setF(true, 7.1); doc.setTextColor(10); text((i + 1) + '.', RULE_X - 1.3, y, { align: 'right' }); }
      doc.setDrawColor(180); doc.setLineWidth(.15);
      doc.line(RULE_X, y - 2.5, RULE_X, y + blockH - 2.5);
      setF(false, 7.1); doc.setTextColor(15);
      chunk.forEach(function (l, li) { textJustified(l, TXT_X, y + li * CLAUSE_LH, TXT_W, idx + li === lines.length - 1); });
      y += blockH;
      idx += chunk.length;
      first = false;
      if (idx < lines.length) ensureSpace(CEIL + 1);
    }
    y += CLAUSE_GAP;
  });
  doc.setTextColor(0);
  ...
  /* ── Подписи - парные рамки, зеркальные «паспортным» рамкам шапки. Печать и подпись
     перекрываются (как на живых документах), высота рамки проверена рендером 22.09. */
  var SBOX_HEAD = 5.6, SBOX_H = 34;
  sectionHeader('ПОДПИСИ СТОРОН', SBOX_H + 4);
  ensureSpace(SBOX_H + 4);
  var sbTop = y;
  function signBoxFrame(x, title) {
    doc.setDrawColor(70); doc.setLineWidth(.28);
    doc.rect(x, sbTop, colW, SBOX_H);
    doc.setFillColor(234, 234, 234);
    doc.rect(x, sbTop, colW, SBOX_HEAD, 'F');
    doc.setDrawColor(70); doc.line(x, sbTop + SBOX_HEAD, x + colW, sbTop + SBOX_HEAD);
    setF(true, 8.6); doc.setTextColor(15);
    text(title, x + PBOX_PAD, sbTop + SBOX_HEAD - 2.1);
  }
  signBoxFrame(M, 'ИСПОЛНИТЕЛЬ');
  signBoxFrame(M + colW + GAP, 'ЗАКАЗЧИК');
  setF(false, 7.6); doc.setTextColor(20);
  var byE = sbTop + SBOX_HEAD + 3;
  text((ent.director_post || 'Генеральный директор'), M + PBOX_PAD, byE);
  text('_______________', M + colW + GAP + PBOX_PAD, byE);
  if (stampUrl) { try { doc.addImage(stampUrl, 'PNG', M + colW - 26, byE - 1, 20, 20); } catch (e) {} }
  if (signUrl) { try { doc.addImage(signUrl, 'PNG', M + colW - 28, byE + 6, 24, 12); } catch (e) {} }
  var nameYE = byE + 19;
  setF(false, 7.6); doc.setTextColor(20);
  text(ent.director || '', M + PBOX_PAD, nameYE);
  text('М.п.', M + PBOX_PAD, nameYE + 4.4);
  text('м.п.', M + colW + GAP + PBOX_PAD, nameYE + 4.4);
  doc.setTextColor(0);
  y = sbTop + SBOX_H + 3;

  /* ── Рамка по периметру + двухстрочный подвал на КАЖДОЙ странице: номер/дата + страница,
     и доказательственная строка (п.1 договора): идентификатор документа, редакция, время
     формирования по Москве, кто сформировал. */
  var totalPages = doc.internal.getNumberOfPages();
  var metaLine = 'Документ ' + (meta.document_id || (docNo + '-v' + (meta.version || 1))) + ' · редакция ' + (meta.version || 1) +
    (meta.generated_at_msk ? ' · сформирован ' + meta.generated_at_msk + ' МСК' : '') +
    (meta.generated_by_name ? ' · ' + meta.generated_by_name : '');
  for (var p = 1; p <= totalPages; p++) {
    doc.setPage(p);
    doc.setDrawColor(140); doc.setLineWidth(.25);
    doc.rect(FRAME_M, FRAME_M, W - FRAME_M * 2, H - FRAME_M * 2);
    doc.setDrawColor(120); doc.setLineWidth(.15);
    doc.line(M, FOOTER_RULE_Y, M + CW, FOOTER_RULE_Y);
    setF(false, 7); doc.setTextColor(120);
    text('Договор-заявка № ' + docNo + ' от ' + docDate, M, FOOTER_TXT_Y);
    text('Страница ' + p + ' из ' + totalPages, M + CW, FOOTER_TXT_Y, { align: 'right' });
    setF(false, 6.3); doc.setTextColor(140);
    text(metaLine, M, FOOTER_TXT2_Y);
    doc.setTextColor(0);
  }
...
function passText(o) {
  ...
  /* 24.09, Влад: марка перед госномером тягача (КАМАЗ/SCANIA/MERSEDES...) - vehicle_marka
     сервер берёт из справочника техники; у наёмной машины её нет - тогда просто госномер. */
  L.push('Тягач: ' + (v.vehicle_gos ? (v.vehicle_marka ? v.vehicle_marka + ' ' : '') + v.vehicle_gos : 'уточнить') + (v.trailer_gos ? ', п/п ' + v.trailer_gos : ''));
...
function renderView(o, who) {
  ...
  $('#op2-d-body').innerHTML = pendBar + stSeg +
    ...
    '<div class="op2-sect"><div class="op2-t">Груз и точки</div>' +
      ...
      (o.delivery_date ? kv('Срок доставки', '<span class="op2-mono">' + esc(dmy(o.delivery_date)) + (o.delivery_time ? ' ' + esc(String(o.delivery_time).slice(0, 5)) : '') + '</span> <span class="op2-sm op2-dim">· из конструктора договора</span>') : '') +
      (o.payment_mode && CONTRACT_PAY_MODES_[o.payment_mode] ? kv('Оплата по договору', esc(CONTRACT_PAY_MODES_[o.payment_mode].label) + (o.payment_mode === 'postpay_docs' && o.payment_days ? ' · ' + esc(o.payment_days) + ' ' + esc(ruWorkDays_(num(o.payment_days))) : '')) : '') +
...
function wireForm() {
  ...
  /* Влад 22.09: «если масса больше 60 т - автоматом должен ставиться Faymonville, других
     тралов на такую массу нет» + следом (живой пример - менеджер вручную поставила
     "Габарит" при 75 т): «более 20 т сразу переводи в негабарит, автоматически». Оба порога
     НЕ новые - тот же w>59 у типа техники и тот же w>20 у габарита, что уже год проверены в
     Калькуляторе (autoPickVeh_/autoDetectCargo_, files/index.html) - переиспользуем, не
     изобретаем параллельные правила. Оба ОДНОСТОРОННИЕ: форсируют переключение ТОЛЬКО вверх
     (Faymonville / Негабарит) при превышении порога - это физика/факт, а не предпочтение -
     но НЕ откатывают обратно при весе ниже порога: лёгкий груз Faymonville возить МОЖЕТ, а
     Негабарит могли выставить и по габаритам (Д×Ш×В), которые вес не отменяет - обратный ход
     был бы тихой перезаписью чужого осознанного выбора, а не исправлением ошибки. */
  function applyWeightAutoRules_(w) {
    var msgs = [];
    var seg = $('#op2-f-eq');
    if (w > 59 && seg && seg.dataset.cur !== EQ_FAYMON_) {
      collapseEqRow(seg, EQ_FAYMON_);
      msgs.push('тип техники - <b>' + esc(EQ_FAYMON_) + '</b>');
    }
    var gabBtn = $('#op2-f-gab .op2-chip[data-gab="Негабарит"]');
    if (w > 20 && gabBtn && !gabBtn.classList.contains('op2-on')) {
      $$('#op2-f-gab .op2-chip').forEach(function (x) { x.classList.toggle('op2-on', x === gabBtn); });
      msgs.push('габарит - <b>Негабарит</b>');
    }
    if (msgs.length) toast('Масса ' + w + ' т: ' + msgs.join(', '));
  }
  $('#op2-f-weight').addEventListener('input', function () { applyWeightAutoRules_(num(this.value)); });
  // та же пара правил - и на УЖЕ СОХРАНЁННОЙ заявке, открытой на правку (живой пример
  // Влада: менеджер сохранила «Габарит» при 75 т руками, ошибка не видна, пока кто-то не
  // тронет поле веса) - проверяем сразу при открытии формы, не только на ввод.
  var wInit = $('#op2-f-weight'); if (wInit && wInit.value) applyWeightAutoRules_(num(wInit.value));
